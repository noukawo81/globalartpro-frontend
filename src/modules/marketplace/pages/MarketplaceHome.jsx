...
export default function MarketplaceHome() {
  ...
  // buy flow: open confirmation modal showing fees then confirm
  const [buyConfirmVisible, setBuyConfirmVisible] = React.useState(false);
  const [buyDetails, setBuyDetails] = React.useState(null);

  function computeFees(amount) {
    const platformRate = parseFloat(process.env.REACT_APP_PLATFORM_FEE) || 0.05;
    const networkRate = parseFloat(process.env.REACT_APP_NETWORK_FEE) || 0.012;
    ...
    return { amount, platformFee, networkFee, sellerProceeds };
  }
  ...
  async function handleBuy(product, overrideToken) {
    ...
    const fees = computeFees(product.price);
    setBuyDetails({ product, sellerId, token, fees, userId });
    setBuyConfirmVisible(true);
  }

  async function confirmBuy() {
    if (!buyDetails) return;
    const { product, sellerId, token, fees, userId } = buyDetails;
    try {
      ...
      if (res && (res.status === 200 || res.status === 201 || res.status === 204 || (res.data && res.data.ok))) {
        if (res.status === 204) console.log('Buy succeeded with 204 (E2E stub)');
      ...
      <div className="marketplace-grid">
        {filtered.length === 0 ? (
          ...
          filtered.map((product) => (
            ...
            >
              {selectedProduct && buyConfirmVisible && buyDetails && buyDetails.product.id === selectedProduct.id && (
                <div className="modal-overlay" onClick={() => setBuyConfirmVisible(false)}>
                  <div className="modal-card" onClick={(e) => e.stopPropagation()}>
                    <h3>Confirmation d'achat</h3>
                    <div style={{ marginTop: 8 }}>
                      <div>Prix : <strong>{buyDetails.fees.amount} {buyDetails.token}</strong></div>
                      <div>Frais réseau (1.2%) : <strong>{buyDetails.fees.networkFee} {buyDetails.token}</strong></div>
                      <div>Commission plateforme (5%) : <strong>{buyDetails.fees.platformFee} {buyDetails.token}</strong></div>
                      <div>Revenu vendeur net : <strong>{buyDetails.fees.sellerProceeds} {buyDetails.token}</strong></div>
                    </div>
                    <div style={{ display: 'flex', gap: 8, marginTop: 12 }}>
                      <button className="ghost" onClick={() => setBuyConfirmVisible(false)}>Annuler</button>
                      <button className="btn gold" onClick={confirmBuy}>Confirmer l'achat</button>
                    </div>
                  </div>
                </div>
              )}
