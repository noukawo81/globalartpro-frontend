              ...
              <div key={a.id} style={styles.thumb} onClick={() => window.open(a.url || a.src || a.image, '_blank')}>
                ...
                {isCurrentArtist && (
                  <div style={{ position: 'absolute', bottom: 6, left: 6, display: 'flex', gap: 6 }}>
                    ...
                    <button style={styles.smallButton} onClick={async (e) => { e.stopPropagation(); if (!api.getToken || !api.getToken()) { setMessage('Connexion requise pour partager.'); alert('Veuillez vous connecter.'); return; } try { await api.getMe(); } catch { setMessage('Veuillez vous reconnecter.'); alert('Veuillez vous reconnecter.'); return; } api.shareToPortal({ artistId: artist.id, mediaId: a.id, title: a.title || 'Partage' }).then(() => setMessage('Partagé sur le portail.')).catch((err) => { console.error('portal share error', err); setMessage(err?.response?.data?.error || 'Échec du partage.'); }); }}>🌍 Partager</button>
                  </div>
                )}
              </div>
            ))}
